...
class RCMessageCell: UITableViewCell {
	...
	func layoutSubviews(_ size: CGSize) {
		...
		let xBubble = rcmessage.incoming ? RCDefaults.bubbleMarginLeft : (widthTable - 5 - size.width)
		viewBubble.frame = CGRect(x: xBubble, y: 20, width: size.width, height: size.height)
